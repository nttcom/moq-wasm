...
# priority-e2e talks to a single relay, so only relay-a (and its redis)
# is needed.
...
# The client asserts that higher-priority tracks finish receiving first, so a
# non-zero exit is the only failure signal needed.
MOQT_E2E_RELAY_URL="$RELAY_URL" cargo run -p priority-e2e
