...
pub fn read_length_and_variable_bytes_from_buffer(buf: &mut BytesMut) -> Result<Vec<u8>> {
    let mut cur = Cursor::new(&buf[..]);
    let ret = read_length_and_variable_bytes(&mut cur);
...
pub fn read_length_and_variable_bytes(buf: &mut std::io::Cursor<&[u8]>) -> Result<Vec<u8>> {
    if buf.remaining() == 0 {
