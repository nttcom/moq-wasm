...
pub fn read_fixed_length_bytes_from_buffer(buf: &mut BytesMut, length: usize) -> Result<Vec<u8>> {
    //! this function is used for x (A) format.
    //! x (A): Indicates that x is A bits long

    let mut cur = Cursor::new(&buf[..]);
    let ret = read_fixed_length_bytes(&mut cur, length);

    buf.advance(cur.position() as usize);

    ret
}
pub fn read_fixed_length_bytes(buf: &mut std::io::Cursor<&[u8]>, length: usize) -> Result<Vec<u8>> {
    if buf.remaining() == 0 {
        bail!("buffer is empty in read_variable_bytes");
    }
...
pub fn write_fixed_length_bytes(value: &Vec<u8>) -> BytesMut {
    //! this function is used for x (A) format.
    //! x (A): Indicates that x is A bits long
    let mut buf = BytesMut::with_capacity(0);
    buf.extend(value);
