...
impl<T: Buf> BufGetExt for T {
    fn try_get_varint(&mut self) -> Result<u64, TryGetError> {
        if let Some(head) = self.chunk().first() {
            let def_head = *head;
            match def_head >> 6 {
...
mod tests {
    mod success {
        ...
        fn try_get_varint_1_byte() {
            let bytes = BytesMut::from(&[3, 4, 5, 6][..]);
        ...
        fn try_get_varint_2_bytes() {
            let bytes = BytesMut::from(&[70, 5, 15, 27][..]);
            let mut bytes = std::io::Cursor::new(&bytes[..]);
            let result = bytes.try_get_varint();
        ...
        fn try_get_varint_4_bytes() {
            let bytes = BytesMut::from(&[128, 2, 0, 0, 8, 5][..]);
            let mut bytes = std::io::Cursor::new(&bytes[..]);
            let result = bytes.try_get_varint();
        ...
        fn try_get_varint_8_bytes() {
            let bytes = BytesMut::from(&[240, 0, 0, 0, 0, 0, 0, 14, 9][..]);
            let mut bytes = std::io::Cursor::new(&bytes[..]);
            let result = bytes.try_get_varint();
        ...
        fn try_get_string_ok() {
            let bytes = BytesMut::from(&[0x04, 0x61, 0x62, 0x63, 0x64][..]);
            let mut bytes = std::io::Cursor::new(&bytes[..]);
