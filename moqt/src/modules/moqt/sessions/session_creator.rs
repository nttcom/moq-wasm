...
use crate::modules::moqt::controls::control_receiver::ControlReceiver;
...
impl<T: TransportProtocol> SessionCreator<T> {
    pub(crate) async fn create_new_connection(
        ...
        let moqt_sender = ControlSender::<T>::new(send_stream);
        let moqt_receiver = ControlReceiver::<T>::new(receive_stream);
