...
use wtransport::quinn::{self, TransportConfig, VarInt};
...
impl MOQTServer {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        if self.underlay == UnderlayType::QUIC {
            self.setup_for_quic().await
        } else if self.underlay == UnderlayType::WebTransport {
            self.setup_for_webtransport().await
        } else {
            bail!("Underlay must be WebTransport, not {:?}", self.underlay);
        }
    }

    async fn setup_for_quic(&self) -> Result<()> {
        let cert = vec![CertificateDer::from_pem_file(&self.cert_path).expect("failed to load cert file.")];
        let key = PrivateKeyDer::from_pem_file(&self.key_path).expect("failed to load key file.");
        ...
        let keep_alive_sec = std::time::Duration::from_secs(self.keep_alive_interval_sec);
    ...
    async fn setup_for_webtransport(&self) -> Result<()> {
        let mut transport_config = TransportConfig::default();
        // 単方向ストリーム数を100000に設定
        transport_config.max_concurrent_uni_streams(100000u32.into());
        // initial_max_stream_data_uniと同義。デフォルトは65,536 バイト (64KB) 大きくするとACKを待たずに送信するため、輻輳が発生する可能性が高まる
