...
use moqt_core::{
    ...
    messages::control_messages::{
        client_setup::ClientSetup, server_setup::ServerSetup, setup_parameters::MaxSubscribeID,
...
fn create_setup_parameters(downstream_max_subscribe_id: u64) -> Vec<SetupParameter> {
...
pub(crate) async fn setup_handler(
    ...
    let setup_parameters = create_setup_parameters(downstream_max_subscribe_id);
...
mod success {
    ...
    use moqt_core::messages::control_messages::{
        ...
        setup_parameters::{Path, SetupParameter},
    };
    use std::vec;
    use tokio::sync::mpsc;

    #[tokio::test]
    async fn path_on_quic() {
...
mod failure {
    ...
    async fn include_unknown_parameter() {
        let senders_mock = senders::test_helper_fn::create_senders_mock();
        let mut client = MOQTClient::new(33, senders_mock);
        let setup_parameters = vec![SetupParameter::Unknown(0)];
        let client_setup_message =
            ClientSetup::new(vec![constants::MOQ_TRANSPORT_VERSION], setup_parameters);
        let underlay_type = crate::constants::UnderlayType::WebTransport;
