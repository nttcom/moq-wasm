...
Set `MOQT_URL` as well when using the MoQ bridge.
...
`make onvif` runs the `moqt-bridge-onvif` GUI binary.
...
- PTZ GUI layout and command handling live in `bridges/onvif/src/ui_ptz.rs`.
- PTZ input defaults and parsing live in `bridges/onvif/src/ui_ptz/inputs.rs`.
- MoQ bridge entrypoint is `bridges/onvif/src/bin/moqt-onvif-client.rs`.
- RTSP encoded packet capture for MoQ lives in `bridges/onvif/src/rtsp_decoder.rs`.
